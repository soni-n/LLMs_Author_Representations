...
def load_user_embeds(embeds, user_id_column='user_id'):
    embeds = embeds['user_representations']
...
def load_doc_embeds(embeds, text_id_column='message_id'):
    embeds = embeds['document_embeddings']
...
def load_token_embeds(embeds, text_id_column='message_id'):
    embeds = embeds['word_embeddings']
...
def generate_embedding_handler(args):
    outputs = None
    if args.mode == 'concat':
...
def generate_csv_handler(embeds, args):
    ...
    if args.return_user_representation_as_mean_user_states or args.return_last_token_as_user_representation:
        ...
        user_embeds = load_user_embeds(embeds, user_id_column=args.user_id_column)
        ...
        token_embeds = load_token_embeds(embeds)
        ...
        doc_embeds = load_doc_embeds(embeds)
        if args.group_by_column != None:
            averaged_embeds = group_doc_embeds_by_column(doc_embeds, args.data, group_by_column=args.group_by_column, text_id_column=args.text_id_column)
            df = get_dlatk_df(averaged_embeds, embed_column='embedding', id_column=args.group_by_column)
        else:
...
if __name__ == '__main__':
    ...
    embeds = generate_embedding_handler(args)

    # Convert to DLATK-compatible CSV
    dlatk_df = generate_csv_handler(embeds, args)

    # Save CSV
    fdir, fname = get_csv_dir(args, args.suffix)
    fdir = os.path.join(args.save_dir, fdir)
    os.makedirs(fdir, exist_ok=True)
    dlatk_df.to_csv(os.path.join(fdir, fname))
